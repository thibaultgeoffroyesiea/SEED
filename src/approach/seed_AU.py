...
class Appr(Inc_Learning_Appr):
    ...
    def create_distributions(self, t, trn_loader, val_loader):
        ...
        for bb_num in range(min(self.max_experts, t+1)):
            ...
            for c in range(classes):
                ...
                class_features = torch.full((2 * len(ds), self.model.num_features + 17), fill_value=-999999999.0, device=self.model.device)
    ...
    def eval(self, t, val_loader):
        ...
        for images, aus ,targets in val_loader:
            ...
            print(features.shape)    
            features  = np.concatenate((features.cpu().numpy(), aus), axis=1)
            features = features.reshape(128, 1, 81)
            features = torch.tensor(features).to(self.device)
    ...
    def predict_class_all_expert(self, features):
        ...
        for bb_num, _ in enumerate(self.experts_distributions):
            ...
            # print("***********DIM1***********")
            # print(softmax_temperature(log_probs[:, bb_num, :], dim=0, tau=self.tau)[0])
